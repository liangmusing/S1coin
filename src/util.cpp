...
const char * const BITCOIN_CONF_FILENAME = "sheepcoin.conf";
const char * const BITCOIN_PID_FILENAME = "sheepcoin.pid";
...
#else
    const char* pszModule = "sheepcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sheepcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Sheepcoin";
#else
    // Unix
    return pathRet / ".sheepcoin";
#endif
#endif
}
