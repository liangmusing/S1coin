Sheepcoin Core version *0.15.1* is now available from:

  <https://download.sheepcoin.org/sheepcoin-0.15.1/>
...
A number of changes to the way Sheepcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lambs per kB (0.00001 SHC). This is to prepare for dropping the minimum transaction fee to 0.1 lambs per kB (0.0001 SHC) in the next release.
